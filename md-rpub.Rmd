...
``` {r quarterly-change-petrol-country-code, warning=FALSE}
...
``` {r quarterly-change-petrol-country-plot, echo=FALSE, warning=FALSE}
ggplot(
  quarterly_country,
  aes(x = pct_change)
) +
...
- The majority of observations were classified as high impact, while low impact fuel price changes occurred less
  frequently.
...
```{r save-models-regression, eval=FALSE}
...
plot_df <- comparison_df |>
  ...
  mutate(
    Model_short = case_when(
      Model == "Random Forest" ~ "RF",
      Model == "Multinomial Logistic" ~ "M-LR",
      Model == "XGBoost" ~ "XGB",
      TRUE ~ Model
    )
  )
...
p1 <- ggplot(plot_df, aes(x = Model_short, y = Value, fill = Model)) +
